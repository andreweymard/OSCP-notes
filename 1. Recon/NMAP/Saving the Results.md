...
```shell-session
astrocat@htb[/htb]$ sudo nmap 10.129.2.28 -p- -oA target
...
```shell-session
astrocat@htb[/htb]$ cat target.nmap
...
```shell-session
astrocat@htb[/htb]$ cat target.gnmap
...
```shell-session
astrocat@htb[/htb]$ cat target.xml
...
```shell-session
astrocat@htb[/htb]$ xsltproc target.xml -o target.html
```
